...
test.describe("Member - Booking History Workflow", () => {
     ...
     */
    test("TC-PKHS-01.1: Verify navigation to Booking History page", async ({ page }) => {
        // Navigate to Booking History
        // Navigate to Booking History
        // 1. Click "Manage Booking" (จัดการการจอง) to expand/access menu
        await page.getByRole('link', { name: 'จัดการการจอง' }).or(page.getByRole('button', { name: 'จัดการการจอง' })).click();
    ...
    test("TC-PKHS-01.2: Search booking history", async ({ page }) => {
        ...
        const firstRowName = await originalRows.first().locator('td').nth(0).innerText();
     ...
     */
    test("TC-PKHS-01.3: Verify booking history table display", async ({ page }) => {
        // Navigate via UI
        await page.getByRole('link', { name: 'จัดการการจอง' }).or(page.getByRole('button', { name: 'จัดการการจอง' })).click();
        const historyLink = page.getByRole('link', { name: 'ประวัติการจอง' }).or(page.getByRole('button', { name: 'ประวัติการจอง' }));
        await historyLink.click();
        ...
        // Check Table Headers
        const headers = ['ชื่อผู้จอง', 'ชื่อกิจกรรม', 'ราคา', 'สถานะ', 'หลักฐาน', 'เวลา'];
