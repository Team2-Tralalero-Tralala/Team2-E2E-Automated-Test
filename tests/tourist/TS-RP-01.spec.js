...
test.describe("Tourist - CreateFeedback", () => {
    ...
    /**
     * TS-RP-01: ผู้ใช้งาน Tourist เขียนรีวิวและให้คะแนนแพ็กเกจ โดยเพิ่มรูป
     */

    /**
     * TS-RP-01.1: ค้นหาด้วยคีย์เวิร์ด (ใช้ช่องค้นหาด้านล่าง)
     * 1.เข้าสู่ระบบบัญชี Tourist 
     * 2.คลิกเมนู "โปรไฟล์" ที่มุมขวาบน
     * 3.เลือกเมนู "ประวัติการจอง"
     * 4.เลือกแพ็กเกจที่ต้องการส่งรีวิว
     * 5. คลิกปุ่มข้อความ (Text Button) "ข้อเสนอแนะ" 
     * 6.ให้คะแนนแพ็กเกจ
     * 7.กรอกข้อเสนอแนะ ไม่เกิน 200 ตัวอักษร
     * 8. เพิ่มรูป ไม่เกิน 5 รูป
     */
    test("TS-RP-01.1: เขียนรีวิว ให้คะแนน และเพิ่มรูปได้สำเร็จ", async ({ page }) => {
        await createFeedbackSuccess(page);
    });

    /**
     * TS-RP-01.2 เขียนรีวิวและให้คะแนนสำเร็จ แต่เพิ่มรูปไม่สำเร็จ
     * 1.เข้าสู่ระบบบัญชี Tourist 
     * 2.คลิกเมนู "โปรไฟล์" ที่มุมขวาบน
     * 3.เลือกเมนู "ประวัติการจอง"
     * 4.เลือกแพ็กเกจที่ต้องการส่งรีวิว
     ...
     * 6.ให้คะแนนแพ็กเกจ
     * 7.กรอกข้อเสนอแนะ ไม่เกิน 200 ตัวอักษร
     * 8. เพิ่มรูป เกิน 5 รูป
     * 9. คลิกปุ่ม "ยืนยัน"
     */
    test("TS-RP-01.2: เขียนรีวิวและให้คะแนนสำเร็จ แต่เพิ่มรูปไม่สำเร็จ", async ({ page }) => {
        await createFeedbackImageFail(page);
    });

    /**
     * TS-RP-01.3: ให้คะแนนและเพิ่มรูปสำเร็จ แต่เขียนรีวิวไม่สำเร็จ
     * 1.เข้าสู่ระบบบัญชี Tourist 
     * 2.คลิกเมนู "โปรไฟล์" ที่มุมขวาบน
     * 3.เลือกเมนู "ประวัติการจอง"
     * 4.เลือกแพ็กเกจที่ต้องการส่งรีวิว
     ...
     * 9. คลิกปุ่ม "ยืนยัน"
     */
    test("TS-RP-01.3: ให้คะแนนและเพิ่มรูปสำเร็จ แต่เขียนรีวิวไม่สำเร็จ", async ({ page }) => {
        await createFeedbackReviewFail(page);
    });

    /**
     * TS-RP-01.4: ให้คะแนน แต่เขียนรีวิวและเพิ่มรูปไม่สำเร็จ
     * 1.เข้าสู่ระบบบัญชี Tourist 
     * 2.คลิกเมนู "โปรไฟล์" ที่มุมขวาบน
     * 3.เลือกเมนู "ประวัติการจอง"
     * 4.เลือกแพ็กเกจที่ต้องการส่งรีวิว
     ...
     * 9. คลิกปุ่ม "ยืนยัน"
     */
    test("TS-RP-01.4: ให้คะแนน แต่เขียนรีวิวและเพิ่มรูปไม่สำเร็จ", async ({ page }) => {
        await createFeedbackReviewAndImageFail(page);
    });
    /*
...
แต่ยังทำให้ผม/ฉันรู้สึกอุ่นใจและได้รับพลังบวกอย่างมาก`;
        ...
        await uploadExtraImages(page, [
            ...
            "assets/photo/IMG_1.jpg",
            "assets/photo/IMG_2.jpg",
        ]);
    ...
    /*
    * คำอธิบาย : ฟังก์ชันทดสอบกรณีการเพิ่มรูปภาพเกิน 5 รูป ทำให้ไม่สามารถส่งข้อเสนอแนะได้
    * Input: page (Page Object)
    * Output : - (แสดงข้อความแจ้งเตือนข้อผิดพลาดเนื่องจากรูปภาพเกินจำนวนที่กำหนด)
    */
    async function createFeedbackImageFail(page) {
        await goToPageCreateFeedback(page);
        await page.getByRole('button').nth(6).click();
        await page.getByRole('textbox', { name: 'ข้อเสนอแนะ' }).click();
        const feedbackText = `ผม/ฉันขอขอบคุณเป็นอย่างยิ่งสำหรับความช่วยเหลือและความเอาใจใส่ที่คุณมอบให้ในครั้งนี้เพราะไม่ใช่แค่ทำให้งานสำเร็จลุล่วงเท่านั้น 
แต่ยังทำให้ผม/ฉันรู้สึกอุ่นใจและได้รับพลังบวกอย่างมาก`;
    ...
    */
    ...
    /*
    * คำอธิบาย : ฟังก์ชันทดสอบกรณีเขียนข้อเสนอแนะเกิน 200 ตัวอักษรและเพิ่มรูปภาพเกิน 5 รูป
    * Input: page (Page Object)
    * Output : - (แสดงข้อความแจ้งเตือนข้อผิดพลาด)
    */
    async function createFeedbackReviewAndImageFail(page) {
        await goToPageCreateFeedback(page);
        await page.getByRole('button').nth(6).click();
        await page.getByRole('textbox', { name: 'ข้อเสนอแนะ' }).click();
        const feedbackText = `ผม/ฉันขอขอบคุณเป็นอย่างยิ่งสำหรับความช่วยเหลือและความเอาใจใส่ที่คุณมอบให้ในครั้งนี้เพราะไม่ใช่แค่ทำให้งานสำเร็จลุล่วงเท่านั้น 
    ...
    /*
    * คำอธิบาย : ฟังก์ชันสำหรับอัปโหลดรูปภาพเพิ่มเติมเข้าสู่ระบบ
    * Input: page (Page Object), filesRelativePaths (Array ของที่อยู่ไฟล์รูปภาพ)
    * Output : - (แสดงรายการไฟล์รูปภาพที่เลือกบนหน้าจอ)
    */
    async function uploadExtraImages(page, filesRelativePaths) {
        const path = require('path');
        const files = filesRelativePaths.map((p) => path.join(process.cwd(), p));
        const container = page.locator('div').filter({ hasText: "เพิ่มรูปภาพ" }).first();
        const input = page.locator('input[type="file"]');
        await expect(input).toBeAttached({ timeout: 10000 });
        await input.setInputFiles(files);
        const removeBtns = container.getByRole("button", { name: "ลบไฟล์ลำดับที่" });
        await expect(removeBtns).toHaveCount(files.length, { timeout: 60000 });
    }
});