 ...
 */
async function goToPageEditCommunityDetail(page, communityTitle) {
  ...
  const clickRow = page.getByRole("link", { name: new RegExp(communityTitle), });
  await expect(clickRow).toBeEnabled();
  await clickRow.click();

  const clickBtn = page.getByRole("button", { name: /แก้ไข/ });
...
async function pickComboboxOption(page, label, optionText) {
  ...
  // เปิด dropdown
  await combo.click();
  // พิมพ์เพื่อค้นหา (ใช้ fill แทน type จะชัวร์กว่า)
  await combo.fill(optionText);
  // เลือกจากรายการ (ถ้ามี role=option)
  const opt = page.getByRole("option", { name: optionText }).first();
  if (await opt.count()) {
    await opt.click();
    return;
  }
  // fallback: บาง UI ไม่ได้ใช้ role=option
...
test.describe("SuperAdmin - Edit Community", () => {
  ...
  test("TS-ECMNT-01.1: แก้ไขและกรอกข้อมูลครบถ้วน (ข้อมูลชุมชน)", async ({ page }) => {
    ...
    const successDialog = page.getByRole("dialog").filter({
      hasText: "แก้ไขวิสาหกิจชุมชนสำเร็จ",
    });
    ...
    // เช็คชื่อใหม่ในตาราง (แนะนำใช้ role=cell)
    const newName = "ชุมชนท่าน้ำ";
    await expect(page.getByRole("cell", { name: newName })).toBeVisible();

    // เช็คว่าไม่มีชื่อเก่าแล้ว
    const oldName = "วิสาหกิจชุมชนท่องเที่ยวเชิงเกษตรบ้านร่องกล้า เทส";
  ...
  test("TS-ECMNT-01.2: ไม่กรอกชื่อวิสาหกิจ", async ({ page }) => {
    await goToPageEditCommunityDetail(
      page,
      "วิสาหกิจชุมชนกลุ่มแม่บ้านเกษตรกรมหาสวัสดิ์ ทดสอบการแก้ไข22"
    );
    ...
    // await nameInput.type("ชุมชนท่าน้ำ");
  ...
  * อัพโหลดไฟล์เกินขนาด
  */
  test("TS-ECMNT-01.7: อัพโหลดไฟล์เกินขนาด", async ({ page }) => {
    await goToPageEditCommunityDetail(
      page,
      "วิสาหกิจชุมชนท่องเที่ยวบ้านน้ำเชี่ยว"
    );
    ...
    const filePath = path.join(
      process.cwd(),
      "assets/photo/large-image.jpg" // สมมติไฟล์ใหญ่
    );

    const fileInput = page.locator('input[type="file"]').first();
    await fileInput.setInputFiles(filePath);

    // ตอนนี้ยังไม่ควร expect error/dialog ใด ๆ
    // แค่ยืนยันว่า test ไม่ crash
  ...
  */
  test("TS-ECMNT-01.8: อัพโหลดไฟล์ที่ไม่ใช่ (JPG/PNG)", async ({ page }) => {
    ...
    // ต้องไม่รับไฟล์
    const filesCount = await fileInput.evaluate(
  ...
  */
  test("TS-ECMNT-01.9: ไม่อัพโหลดโลโก้", async ({ page }) => {
    ...
    //สำหรับตรวจว่ามีการเรียก upload หรือไม่
    let uploadCalled = false;

    /**
     * ดัก request ที่ยิงออกจากหน้า
     * ถ้ามี POST ไปที่ upload ให้ถือว่ามีการอัปโหลด
     */
    ...
    // สรุป: ต้องไม่มีการเรียก upload ใด ๆ
  ...
  */
  test("TS-ECMNT-01.13: กรอกชื่อกิจกรรมหลัก", async ({ page }) => {
    ...
    await openAccordion(page, "ข้อมูลชุมชน");
    await page.getByLabel("ชื่อกิจกรรมหลัก *").fill("ดูนก");
  ...
  */
  test("TS-ECMNT-01.15: อัพโหลดรูปภาพเพิ่มเติม", async ({ page }) => {
    ...
    // รอผลลัพธ์: สำเร็จ หรือ error
    const successDialog = page.getByRole("dialog").filter({
      hasText: "แก้ไขวิสาหกิจชุมชนสำเร็จ",
    });
    const errorDialog = page.getByRole("dialog").filter({
      hasText: /ข้อมูลไม่ถูกต้อง|เกิดข้อผิดพลาด/,
    });

    await Promise.race([
      successDialog.waitFor({ state: "visible", timeout: 60000 }),
      errorDialog.waitFor({ state: "visible", timeout: 60000 }),
    ]);

    // ถ้าเจอ error -> ให้ fail ด้วยข้อความที่อ่านง่าย
    if (await errorDialog.isVisible()) {
      const msg = (await errorDialog.innerText().catch(() => "")).trim();
      throw new Error(`Save failed (error dialog shown): ${msg || "unknown error"}`);
    }
    ...
    // บางครั้งระบบอาจไม่ redirect ทันที ให้เผื่อ timeout
    await expect(page).toHaveURL(/super\/communities(\/all)?/, { timeout: 60000 });
  });
  ...
  */
  test("TS-ECMNT-01.16: อัพโหลดวิดีโอเพิ่มเติม", async ({ page }) => {
    await goToPageEditCommunityDetail(page, "วิสาหกิจชุมชนชีววิถีตำบลน้ำเกี๋ยน");
    await openAccordion(page, "ข้อมูลชุมชน");
    ...
    // รอให้ปุ่มบันทึกกดได้ (กันกรณีวิดีโอยังอัปโหลดไม่เสร็จ)
    const saveBtn = page.getByRole("button", { name: "บันทึก" });
    await expect(saveBtn).toBeEnabled({ timeout: 60000 });
    await saveBtn.click();
  ...
  */
  test("TS-ECMNT-01.21: ปักหมุดวิสาหกิจชุมชน", async ({ page }) => {
    ...
    await clickMapAndWaitLatLng(page);

    // ถ้าระบบยังต้องกด “ปักหมุด” เพื่อยืนยันค่อยกด (แต่ให้ locator แบบ getByText)
  ...
  */
  test("TS-ECMNT-01.23: กรอกข้อมูลครบถ้วน (ข้อมูลติดต่อและผู้ดูแล)", async ({ page }) => {
    ...
    openAccordion(page, "ข้อมูลติดต่อและผู้ดูแล")
    ...
    // บาง UI ต้องกด Enter เพื่อให้ dropdown แสดง
    await memberCombo.press("Enter");

    // คลิก “แถวรายการ” ของชื่อ (เสถียรกว่าไปกด checkbox)
    const row = page.getByText("จินตนา จิรายุ", { exact: true });
    await expect(row).toBeVisible();
    await row.click();

    // ปิด dropdown ก่อน (ลด ambiguity)
