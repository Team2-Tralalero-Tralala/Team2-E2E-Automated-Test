...
test.describe("Admin - Edit Community", () => {
  ...
  /**
   * TS-EHT-03.1
   * Admin แก้ไขที่พักได้ เมื่อกรอกข้อมูลครบ
   */
  test("TS-EHT-03.1: Admin แก้ไขที่พักได้ เมื่อกรอกข้อมูลครบ", async ({ page }) => {
  ...
  /**
     * TS-EHT-03.2
     * Admin ไม่สามารถแก้ไขที่พักได้ เมื่อกรอกข้อมูลไม่ครบ
     */
    test("TS-EHT-03.2: Admin ไม่สามารถแก้ไขที่พักได้ เมื่อกรอกข้อมูลไม่ครบ", async ({ page }) => {
      await goToEditHomestayDetailPage(page);
      await page.getByRole("textbox", { name: "ชื่อที่พัก *" }).click();
      await page
        .getByRole("textbox", { name: "ชื่อที่พัก *" })
     ...
     */
    test("TS-EHT-03.3: คลิกปุ่ม (Button) \"ยกเลิก\" ในหน้้าต่างแสดงผลซ้อนระบบไม่บันทึกข้อมูล", async ({ page }) => {
      await goToEditHomestayDetailPage(page);
      await page.getByRole("textbox", { name: "ชื่อที่พัก *" }).click();
      await page
        .getByRole("textbox", { name: "ชื่อที่พัก *" })
      ...
      const confirmButton = page.getByRole("button", { name: "ยกเลิก" });
      await expect(confirmButton).toBeVisible();
      await confirmButton.click();
    });
    /**
     * TS-EHT-03.4
     * คลิกปุ่ม (Button) "ยกเลิก" ในหน้าแก้ไขที่พักระบบกลับไปยังหน้า "จัดการที่พัก"
     */
    test("TS-EHT-03.4: คลิกปุ่ม (Button) \"ยกเลิก\" ในหน้าแก้ไขที่พักระบบกลับไปยังหน้า \"จัดการที่พัก\"", async ({ page }) => {
      await goToEditHomestayDetailPage(page);
    await page.getByRole("textbox", { name: "ชื่อที่พัก *" }).click();
    await page
      .getByRole("textbox", { name: "ชื่อที่พัก *" })
