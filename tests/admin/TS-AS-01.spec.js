...
test.describe("Admin - Edit Community", () => {
  ...
  async function fillFormCorrect(page) {
    ...
    await page
      .getByRole("textbox", {
        name: "ป้อนชื่อวิสาหกิจชุมชนหรือสถานที่ใกล้เคียงเพื่อปักหมุด",
      })
      .click();
    await page
      .getByRole("textbox", {
      ...
      .fill("มหาวิทยาลัยบูรพา");
    await page
      .getByRole("listitem")
      .filter({ hasText: "มหาวิทยาลัยบูรพา, 169" })
      .click();
    await page
   ...
   */
  //TS-AS-01.2: กรอกข้อมูลไม่ครบถ้วนหลายจุด
   ...
   */
  //TS-AS-01.3 ปักหมุดหากไม่พบสถานที่
