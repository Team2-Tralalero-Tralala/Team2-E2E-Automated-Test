...
async function goToPageHomestay(page) {
  ...
  const manageHomestay = page.getByRole("link", { name: "จัดการที่พัก" });
  await expect(manageHomestay).toBeVisible();
  await manageHomestay.click();
...
test.describe("Admin - Add Homestay", () => {
   ...
   * กรอกข้อมูลครบ
   */
  test("TC-AHT-03.1: Admin add homestay successfully", async ({
    page,
  }) => {
   ...
   * กรอกไม่ข้อมูลครบ
   */
  test("TC-AHT-03.2: Admin add homestay with incomplete data", async ({
    page,
  }) => {
    await goToPageHomestay(page);
   ...
   */
  test("TC-AHT-03.3: Admin add homestay with pinning location", async ({
    page,
  }) => {
    await goToPageHomestay(page);
   ...
   * เพิ่มแท็ก
   */
  test("TC-AHT-03.4: Admin add homestay with tags", async ({
    page,
  }) => {
    await goToPageHomestay(page);
   ...
   * อัพโหลดรูปภาพ
   */
  test("TC-AHT-03.5: Admin add homestay upload image", async ({
    page,
  }) => {
    await goToPageHomestay(page);
