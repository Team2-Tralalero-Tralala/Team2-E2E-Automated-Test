...
test.describe("Admin - Edit Community", () => {
   ...
   */
  test('TS-DHT-02.1:คลิกปุ่มไอคอน (Icon Button) "ถังขยะ"ต้องโชว์หน้าต่างแสดงผลซ้อน (Modal)', async ({
    page,
  }) => {
    const manageHomestays = page.getByRole("link", { name: "จัดการที่พัก" });
    await expect(manageHomestays).toBeVisible();
    ...
    await page
      .getByRole("row", {
        name: "บางแสนริมเล แอร์ น้ำอุ่น อาหารเช้า โฮมสเตย์ แก้ไข ลบ",
      })
      .getByLabel("ลบ")
      .click();
  });
  ...
  /**
   * TS-DHT-02.2
   * คลิกปุ่ม (Button) "ยืนยัน" ในหน้าต่างแสดงผลซ้อน (Modal) ที่พักจะถูกลบ
   */
  test('TS-DHT-02.2: คลิกปุ่ม (Button) "ยืนยัน" ในหน้าต่างแสดงผลซ้อน (Modal) ที่พักจะถูกลบ', async ({
    page,
  }) => {
    const manageHomestays = page.getByRole("link", { name: "จัดการที่พัก" });
    await expect(manageHomestays).toBeVisible();
    ...
    await page.getByRole("textbox", { name: "ชื่อที่พัก *" }).fill("ทดสอบลบ");
    await page.getByRole("textbox", { name: "ประเภทที่พัก *" }).click();
    await page.getByRole("textbox", { name: "ประเภทที่พัก *" }).fill("ทดสอบลบ");
    ...
    await page.getByRole("textbox", { name: "บ้านเลขที่ *" }).fill("1");
    ...
    await page.getByRole("option", { name: "บางปลาสร้อย" }).click();
    await page.getByRole("textbox", { name: "คำอธิบายที่อยู่" }).click();
    await page.getByRole("textbox", { name: "คำอธิบายที่อยู่" }).fill("555555");
    await page.getByText("ปักหมุด", { exact: true }).click();
    await page.getByRole("button", { name: "บันทึก" }).click();
    await page.getByRole("button", { name: "ยืนยัน" }).click();
    await page
      .getByRole("row", { name: "ทดสอบลบ ทดสอบลบ ทดสอบลบ แก้ไข ลบ" })
      .getByLabel("ลบ")
      .click();
    await page.getByRole("button", { name: "ยืนยัน" }).click();
  });
   ...
   /**
   * TS-DHT-02.3
   * คลิกปุ่ม (Button) "ยกเลิก" ในหน้าต่างแสดงผลซ้อน (Modal) ที่พักจะไม่ถูกลบ
   */
  test('TS-DHT-02.3: คลิกปุ่ม (Button) "ยกเลิก" ในหน้าต่างแสดงผลซ้อน (Modal) ที่พักจะไม่ถูกลบ', async ({
    page,
  }) => {
    const manageHomestays = page.getByRole("link", { name: "จัดการที่พัก" });
    await expect(manageHomestays).toBeVisible();
