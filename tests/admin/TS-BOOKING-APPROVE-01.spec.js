...
test.describe("Admin - Booking Approval Workflow", () => {
  ...
  test.describe("Authorized Admin Actions", () => {
     ...
     * 5. ยืนยันการอนุมัติ
     */
    test("TC-BOOKING-APPROVE-01.1: Approve 'Pending' Booking", async ({
      page,
    }) => {
      // 1. Find "Pending" row
      const pendingRow = page
        .locator("tr")
        .filter({ has: page.getByText("รอตรวจสอบ") })
        .first();
      ...
      const confirmButton = page
        ...
        .or(page.getByRole("button", { name: "ตกลง" }))
      ...
      await expect(
        page
          ...
          .filter({ hasText: rowText.split("\n")[0] }),
      ).toHaveCount(0); // Rough check
    });
     ...
     */
    test("TC-BOOKING-APPROVE-01.2: Cancel Approval in Popup", async ({
      page,
    }) => {
      ...
      if ((await pendingRow.count()) === 0) {
        test.skip(
          "No 'Pending' (รอตรวจสอบ) bookings available for testing cancel popup.",
        );
        return;
      }
     ...
     */
    test("TC-BOOKING-APPROVE-01.3: Verify approved booking in 'Booking History'", async ({
      page,
    }) => {
      // 1. Go to History
      ...
      const filterButton = page
        .locator("button")
        .filter({ hasText: /สถานะ|Status/ })
        .last();
      if (await filterButton.isVisible()) {
        await filterButton.click();
        const successOption = page
          .getByRole("option", { name: "จองสำเร็จ" })
  ...
  test.describe("Unauthorized Access", () => {
     ...
     */
    test("TC-BOOKING-APPROVE-01.4: Member cannot access Manage Bookings page", async ({
      page,
    }) => {
      // 1. Login as Member
