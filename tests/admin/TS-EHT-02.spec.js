...
async function goToEditHomestayDetailPage(page, target = "บางแสนริมเล") {
 await page.getByRole('link', { name: 'จัดการชุมชน' }).click();
  await page.getByRole('button', { name: 'ที่พัก จำนวน 3 ที่พัก' }).click();
  await page.getByRole('button', { name: 'จัดการ' }).click();
  await page.getByRole('row', { name: 'บางแสนริมเล แอร์ น้ำอุ่น อาหารเช้า โฮมสเตย์ แก้ไข ลบ' }).getByLabel('แก้ไข').click();
}
...
test.describe("Admin - Edit Community", () => {
   ...
   */
  test("TS-EHT-02.1: กรอกข้อมูลที่ต้องการแก้ไขครบถ้วน", async ({ page }) => {
   ...
   */
  test("TS-EHT-02.2: กรอกข้อมูลไม่ครบถ้วนหลายจุด", async ({ page }) => {
    await goToEditHomestayDetailPage(page);
    await page.getByRole("textbox", { name: "ชื่อที่พัก *" }).click();
    await page
      .getByRole("textbox", { name: "ชื่อที่พัก *" })
   ...
   */
  test("TS-EHT-02.3: ปักหมุดหากไม่พบสถานที่", async ({ page }) => {
    await goToEditHomestayDetailPage(page);
   ...
   * แก้ไขแท็ก
   */
  test("TS-EHT-02.4: แก้ไขแท็ก", async ({ page }) => {
    await goToEditHomestayDetailPage(page);
    await page
      .getByRole("combobox", { name: "ค้นหาแท็ก เช่น เดินป่า ทะเล ภูเขา" })
      .click();
