...
/**
 * clickMemberInfo - คลิกที่สมาชิก
 */
async function clickMemberInfo(page) {
  await page.getByRole("row", { name: "แดง ชาติ" }).click();
}
/**
 * submitForm - บันทึกฟอร์ม
 */
...
test.describe("admin - Edit Activity Role Member", () => {
  ...
  /**
   * TS-EMT-02.1: กรอกบทบาทวิสาหกิจ
   */
  test("TS-EMT-02.1: กรอกบทบาทวิสาหกิจ", async ({ page }) => {
    await clickMemberInfo(page);
  ...
  /**
   * TS-EMT-02.2:ไม่กรอกบทบาทวิสาหกิจ
   */
  test("TS-EMT-02.2:ไม่กรอกบทบาทวิสาหกิจ", async ({ page }) => {
    await clickMemberInfo(page);
    await page.getByRole("button", { name: "แก้ไข" }).click();
